...
then
  ...
  then
    ...
    do
      ${SSH} -o ConnectTimeout=5 core@okd4-worker-${i}.${SUB_DOMAIN}.${LAB_DOMAIN} "sudo wipefs /dev/${boot_dev} && sudo dd if=/dev/zero of=/dev/${boot_dev} bs=512 count=1 && sudo poweroff"
...
then
  ...
  then
    ...
    do
      ...
      ${SSH} -o ConnectTimeout=5 core@okd4-master-${i}.${SUB_DOMAIN}.${LAB_DOMAIN} "sudo wipefs /dev/${boot_dev} && sudo dd if=/dev/zero of=/dev/${boot_dev} bs=512 count=1 && sudo poweroff"
    done
  else
    for i in 0 1 2
    do
